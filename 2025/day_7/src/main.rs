...
struct Grid<T> {
    height: usize, // y
    width: usize,  // x
    cells: Vec<T>, // Use a flat vec, we can calculate the indices based on the height / width
}
impl<T> Grid<T> {
    fn new(height: usize, width: usize, cells: Vec<T>) -> Self {
        Self {
            height,
            width,
            cells,
    ...
    fn set_left(&mut self, coord: Coord, value: T) {
    ...
    fn iter_values(&self) -> impl Iterator<Item = (Coord, Option<T>)>
    where
        T: Copy,
    {
    ...
    fn above(&self, coord: &Coord) -> Option<(Coord, T)>
    where
        T: Copy,
    {
...
fn solve_part2(input: &str) -> usize {
    ...
    fn split_world(grid: &Grid<char>, starting_idx: usize, mut count: usize) -> usize {
        let mut new_grid = grid.clone();
        grid.iter_values()
            .enumerate()
            .skip(starting_idx)
