...
def touching_box(previous_pos, movement):
    previous_box = get_box(board[previous_pos], previous_pos)
    for pos, _ in previous_box:
        if board[add_coord(pos, movement)] in ["[", "]"]:
            return True
    return False

def get_box(item, pos):
    if item == "[":
        return [(pos, "["), (add_coord((1, 0), pos), "]")]
    elif item == "]":
        return [(pos, "]"), (add_coord((-1, 0), pos), "[")]
    return []
...
for i, move in enumerate(moves):
    ...
    current_item = board[checking_pos]
    ...
    if isinstance(current_item, Box):
        boxes_touched: list[Box] = [current_item]
        current_boxes: list[Box] = [current_item]
        touched_boxes = [current_item]
        while True:
            checking_pos = add_coord(checking_pos, movement)
            current_item = board[checking_pos]
            for current_box in current_boxes:
                touched_boxes.extend(current_box.is_touching(board, movement))
            if touched_boxes:
                current_boxes = touched_boxes
                boxes_touched.extend(touched_boxes)
                touched_boxes = []
                if move == "<":
                    checking_pos = add_coord(boxes_touched[-1].left_coord, movement)
                if move == ">":
                    checking_pos = add_coord(boxes_touched[-1].right_coord, movement)
            else:
                break
    ...
    if current_item == "#":
        ...
        continue
    if current_item == ".":
        # Move all the box pieces by one
        for box in reversed(list(dict.fromkeys(boxes_touched))):
            if move in ["^", "v"]:
                board[box.coord_tuple()[0]] = "."
                board[box.coord_tuple()[1]] = "."
            elif move == ">":
                board[box.coord_tuple()[0]] = "."
            elif move == "<":
                board[box.coord_tuple()[1]] = "."
            box.move(movement)
            board[box.coord_tuple()[0]] = box
            board[box.coord_tuple()[1]] = box
        # Move the robot
        board[robot_pos] = "."
        robot_pos = add_coord(robot_pos, move_to_movement[move])
        board[robot_pos] = "@"
